...
12. Tech Stack and Use Case
13. SMTP Email Delivery
14. Migrations (Full List)
...
When a known issue is detected and unassigned, the engine attempts auto remediation (header/alias fixes, dedupe, re-encoding) and requeues the pipeline. If the rerun succeeds, the incident auto resolves.
...
Scheduling details:
- Schedules are stored on `Job.schedule_cron` and dispatched by `rq-scheduler`.
- Jobs without a cron can still be run manually with “Run batch now”.
- The scheduler container reads cron strings and enqueues execution into the default RQ queue.
- Each schedule execution creates a `JobRun` record with timing, status, and logs.
...
See `TECH_STACK.md` for the technology stack and the school/college batch processing scenario used for this project.

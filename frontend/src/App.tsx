...
type RcaStage = "triage" | "investigation" | "root_cause" | "verification"
type RcaData = {
  stage: RcaStage
  severity: string
  category: string
...
const RCA_STEPS: Array<{ key: RcaStage; label: string; hint: string }> = [
  { key: "triage", label: "Triage & impact", hint: "Capture scope, severity, and immediate containment." },
  { key: "investigation", label: "Investigation", hint: "Log symptoms, evidence, and hypotheses." },
  { key: "root_cause", label: "Root cause", hint: "Confirm cause and corrective/preventive actions." },
  { key: "verification", label: "Verification & follow-up", hint: "Validate fixes and record lessons." },
]
...
function parseRcaNotes(notes?: string | null): RcaData | null {
  if (!notes) return null
  try {
    const parsed = JSON.parse(notes)
    if (!parsed || typeof parsed !== "object") return null
    const rca = (parsed as any).rca
    if (!rca || typeof rca !== "object") return null
    const merged: RcaData = { ...DEFAULT_RCA, ...rca }
    if (!Array.isArray(merged.whys) || merged.whys.length !== 5) {
      merged.whys = [...DEFAULT_RCA.whys]
    }
    return merged
  } catch {
    return null
...
function buildRcaNotes(rca: RcaData) {
...
function formatRcaStage(stage?: string) {
  const match = RCA_STEPS.find((item) => item.key === stage)
  return match ? match.label : "In review"
}
...
const IST_DATE_TIME_PARTS_FORMATTER = new Intl.DateTimeFormat(INDIAN_LOCALE, {
  timeZone: IST_TIMEZONE,
  year: "numeric",
  month: "2-digit",
  day: "2-digit",
...
function parseDateInput(input?: string): Date | null {
  ...
  if (!/[tT]/.test(value) && /^\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}/.test(value)) {
...
function getISTDateTimeParts(date: Date) {
  const parts = IST_DATE_TIME_PARTS_FORMATTER.formatToParts(date)
  let year = 0
  let month = 0
  let day = 0
  let hour = 0
...
function getISTDateTime(date: Date) {
...
function formatDurationShort(ms?: number | null) {
  ...
  return `${(ms / 86400000).toFixed(1)} day`
}
...
async function downloadUploadSource(upload: any) {
  const uploadId = upload?.upload_id ?? upload?.id
  if (!uploadId) {
    alert("Missing intake ID.")
    return
  }
  const token = localStorage.getItem("jwt_token")
  try {
    const res = await fetch(`${baseURL()}/api/uploads/${uploadId}/source/`, {
      method: "GET",
      headers: {
        ...(token ? { Authorization: `Token ${token}` } : {}),
      },
    ...
    const filename =
      getFilenameFromDisposition(header) || upload?.filename || `upload-${String(uploadId).slice(0, 8)}.csv`
    const link = document.createElement("a")
    link.href = window.URL.createObjectURL(blob)
    ...
    alert("Unable to download the source file right now. Please try again.")
  }
}
...
function DashboardPage() {
  ...
  const [incidentPreview, setIncidentPreview] = useState<any[]>([])
  const [moderatorFocus, setModeratorFocus] = useState("unassigned")

  const [health, setHealth] = useState<Record<string, string> | null>(null)

  const moderatorRows = useMemo(() => {
    if (!incidentPreview.length) return []
    const username = auth.user?.username || ""
    if (moderatorFocus === "mine") {
      return incidentPreview.filter((r) => String(r.assignee || "") === username && r.state !== "resolved")
    }
    if (moderatorFocus === "recent") {
      return incidentPreview
        .filter((r) => r.state === "resolved")
        .sort((a, b) => String(b.resolved_at || b.updated_at || "").localeCompare(String(a.resolved_at || a.updated_at || "")))
    }
    return incidentPreview.filter((r) => !r.assignee && r.state !== "resolved")
  }, [incidentPreview, moderatorFocus, auth.user?.username])
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const uploadsToday = uploads.filter((u) => isToday(u.received_at)).length
        const runsToday = runs.filter((r) => isToday(r.started_at || r.finished_at)).length
        const localToday = Math.max(uploadsToday, runsToday)
        setTodayUploads(localToday)
        ...
        setIncidentPreview(incidents)

        // last resolution duration (best-effort)
        ...
        if (resolved.length > 0) {
          ...
          if (Number.isFinite(start) && Number.isFinite(end) && end >= start) {
            ...
            setLastMttr(formatDurationShort(durationMs))
          } else {
            setLastMttr("—")
          }
        } else {
  ...
  const showAdminDetails = auth.role === "admin"
  const showModeratorExtras = auth.role === "moderator"
  const showUserExtras = auth.role === "user"
  ...
  return (
    <div className="page">
      ...
      <div className="kpiGrid">
        <Card>
          <KpiCard label="TODAY'S BATCHES" value={todayUploads} hint="Batch runs today (uploads + schedules)" />
        </Card>
        <Card>
        ...
        <Card>
          <KpiCard label="LAST RESOLUTION DURATION" value={lastMttr} hint="Time to close the most recent issue" />
        </Card>
      </div>
      ...
      {showAdminDetails ? (
        <>
          <div className="grid2">
            <Card>
              <CardTitle
                title="How batches are handled"
                right={<span className="miniPill">Intake + Check + Improve + Share</span>}
              />
        ...
        <div className="grid2" style={{ marginTop: 16 }}>
          <Card>
            <div className="sectionTitle">Issue focus</div>
            <Segments
              value={moderatorFocus}
              onChange={setModeratorFocus}
              options={[
            ...
            <div style={{ display: "grid", gap: 10, marginTop: 12 }}>
              {moderatorRows.length === 0 ? (
                <div className="muted">No issues in this view.</div>
              ) : (
                moderatorRows.slice(0, 4).map((row: any) => (
                  <div key={row.incident_id ?? row.id} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <span className="mono">{String(row.incident_id ?? "—").slice(0, 8)}</span>
                    <span className={cx("statusChip", row.state === "open" ? "bad" : row.state === "resolved" ? "ok" : "")}>
                      {row.state}
                    </span>
                    <span className="muted">{fmtDate(row.created_at)}</span>
                  </div>
                ))
              )}
            </div>
          ...
          <Card>
            <div className="sectionTitle">Quality checklist</div>
            <ul className="plainList">
              <li>Confirm required columns exist (student_id, score).</li>
              <li>Check for duplicate IDs or blank scores.</li>
              <li>Close incidents with clear resolution notes.</li>
            </ul>
          </Card>
        </div>
      ) : showUserExtras ? (
        <div className="grid2" style={{ marginTop: 16 }}>
          <Card>
            ...
            <ul className="plainList">
              <li>Use clear file names so your reports are easy to find later.</li>
              <li>Batch runs show live progress while the engine works.</li>
              <li>Download CSV or PDF once a batch is published.</li>
            </ul>
          </Card>
          <Card>
            <div className="sectionTitle">Daily checklist</div>
            <ul className="plainList">
              <li>Upload today’s files in Batch Intake.</li>
              <li>Confirm the batch finishes without errors.</li>
              <li>If something fails, note the issue and try again.</li>
            </ul>
          </Card>
        </div>
      ) : null}
                    ...
                    <div>
                      ...
                      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                        ...
                        <button
                          ...
                          type="button"
                          onClick={() => downloadUploadSource(upload)}
                        >
                          Download source
                        </button>
                      </div>
...
function IncidentsPage() {
  ...
  const [newErrorText, setNewErrorText] = useState("")
    ...
    const [showRcaWorkspace, setShowRcaWorkspace] = useState(false)
    const [rcaDraft, setRcaDraft] = useState<RcaData>(DEFAULT_RCA)
    const [rcaIncident, setRcaIncident] = useState<any | null>(null)
    ...
    const updateRca = (patch: Partial<RcaData>) => {
      setRcaDraft((prev) => ({ ...prev, ...patch }))
    }

    const updateWhy = (index: number, value: string) => {
      setRcaDraft((prev) => {
        const next = [...(prev.whys || DEFAULT_RCA.whys)]
        next[index] = value
        return { ...prev, whys: next }
      })
    }
  ...
  async function resolveIncident(incidentId?: string) {
    ...
    async function analyzeIncident(incidentId?: string, incident?: any) {
      if (!auth.canManageIncidents) {
        alert("Only moderators and admins can update RCA.")
        return
      }
      if (!incidentId) {
        alert("Missing issue ID.")
      ...
      setRcaError(null)
      setRcaSaving(false)
      setShowRcaWorkspace(true)
      let detail = incident
      try {
        if (!detail || !detail.analysis_notes) {
          detail = await apiClient.incidents.get(String(incidentId))
        }
      } catch {
        setRcaError("Couldn't load the latest issue details.")
      }
      if (!detail) return
      const parsed = parseRcaNotes(detail.analysis_notes)
      const fallbackNotes = !parsed && detail.analysis_notes ? String(detail.analysis_notes) : ""
      const merged: RcaData = {
        ...DEFAULT_RCA,
        ...parsed,
        severity: detail.severity || parsed?.severity || DEFAULT_RCA.severity,
        category: detail.category || parsed?.category || "",
        detection_source: detail.detection_source || parsed?.detection_source || "",
        impact_summary: detail.impact_summary || parsed?.impact_summary || "",
        root_cause: detail.root_cause || parsed?.root_cause || "",
        corrective_action: detail.corrective_action || parsed?.corrective_action || "",
        resolution_report: detail.resolution_report || parsed?.resolution_report || "",
        notes: parsed?.notes || fallbackNotes || "",
      }
      setRcaDraft(merged)
      setRcaIncident(detail)
    }

    async function saveRcaWorkspace() {
      if (!rcaIncident) return
      const incidentId = rcaIncident?.incident_id ?? rcaIncident?.id
      if (!incidentId) return
      try {
        setRcaSaving(true)
        await postJSON(`/api/incidents/${incidentId}/analyze/`, {
          severity: rcaDraft.severity,
          category: rcaDraft.category.trim() || undefined,
          detection_source: rcaDraft.detection_source.trim() || undefined,
          impact_summary: rcaDraft.impact_summary.trim() || undefined,
          analysis_notes: buildRcaNotes(rcaDraft),
          root_cause: rcaDraft.root_cause.trim() || undefined,
          corrective_action: rcaDraft.corrective_action.trim() || undefined,
          resolution_report: rcaDraft.resolution_report.trim() || undefined,
          rca_stage: rcaDraft.stage,
        })
        setShowRcaWorkspace(false)
        setRefreshTick((x) => x + 1)
      } catch {
        setRcaError("Failed to save RCA details. Please try again.")
      } finally {
        setRcaSaving(false)
      }
    }
  ...
  async function archiveIncident(incidentId?: string) {
    ...
    const parsedRca = inspectingIncident ? parseRcaNotes(inspectingIncident.analysis_notes) : null
    const analysisNotesText = parsedRca
      ? parsedRca.notes || ""
      : inspectingIncident?.analysis_notes
      ? String(inspectingIncident.analysis_notes)
      : ""
      ...
      <Card>
        ...
        <div className="tableWrapper">
          <div className="table">
            <div className="tHead">
              ...
              <div>Assignee</div>
              <div>Known issue?</div>
              {auth.role !== "user" ? <div>Actions</div> : null}
            </div>
            ...
            {rows.length === 0 ? (
              ...
              visibleIncidents.map((r) => {
                ...
                const isAssignedToOther = Boolean(assignee && assignee !== auth.user?.username)
        ...
        >
          <div className="detailActionRow">
            <button
              ...
              disabled={!auth.canManageIncidents}
                onClick={() => analyzeIncident(String(inspectingIncident.incident_id), inspectingIncident)}
                title={auth.canManageIncidents ? "" : "Moderators and admins can update RCA."}
              >
                RCA workspace
              </button>
            <button
          ...
          <div className="detailSection">
            ...
            {parsedRca ? (
              <div className="detailSection">
                <div className="detailTitle">RCA workflow</div>
                <div className="rcaSummaryGrid">
                  <div>
                    <div className="muted">Stage</div>
                    <div>{formatRcaStage(parsedRca.stage)}</div>
                  </div>
                  <div>
                    <div className="muted">Detection source</div>
                    <div>{parsedRca.detection_source || inspectingIncident.detection_source || "-"}</div>
                  </div>
                  <div>
                    <div className="muted">Impact</div>
                    <div>{parsedRca.impact_summary || inspectingIncident.impact_summary || "-"}</div>
                  </div>
                  <div>
                    <div className="muted">Root cause</div>
                    <div>{parsedRca.root_cause || inspectingIncident.root_cause || "-"}</div>
                  </div>
                  <div>
                    <div className="muted">Corrective action</div>
                    <div>{parsedRca.corrective_action || inspectingIncident.corrective_action || "-"}</div>
                  </div>
                  <div>
                    <div className="muted">Preventive action</div>
                    <div>{parsedRca.preventive_action || "-"}</div>
                  </div>
                </div>
              </div>
            ) : null}
            ...
            <div className="detailSection">
              ...
              <div className="detailTextBlock">{analysisNotesText || "Not recorded yet."}</div>
            </div>
        ...
        {showRcaWorkspace ? (
          <DetailModal
            ...
            subtitle={
              rcaIncident ? (
                <span className="muted">
                  Issue {String(rcaIncident.incident_id ?? rcaIncident.id ?? "").slice(0, 8)} •{" "}
                  {clampText(rcaIncident.error ?? "", 80) || "Batch issue"}
                </span>
              ) : null
            }
            onClose={() => setShowRcaWorkspace(false)}
            className="rcaModal"
          >
            <div className="rcaIntro">
              <div className="rcaMeta">
                <div>
                  <div className="muted">Severity</div>
                  <div className="statusChip">{rcaDraft.severity}</div>
                </div>
                <div>
                  <div className="muted">Current stage</div>
                  <div>{formatRcaStage(rcaDraft.stage)}</div>
                </div>
                <div>
                  <div className="muted">Assignee</div>
                  <div>{rcaIncident?.assignee || "Unassigned"}</div>
                </div>
              </div>
            </div>

            <div className="rcaStepper">
              {RCA_STEPS.map((step, idx) => (
                <button
                  key={step.key}
                  type="button"
                  className={cx("rcaStep", rcaDraft.stage === step.key && "rcaStep-active")}
                  onClick={() => updateRca({ stage: step.key })}
                >
                  <span className="rcaStepBadge">{idx + 1}</span>
                  <div>
                    <div className="rcaStepLabel">{step.label}</div>
                    <div className="rcaStepHint">{step.hint}</div>
                  </div>
                </button>
              ))}
            </div>

            <div className="rcaPanel">
              {rcaDraft.stage === "triage" ? (
                <div className="rcaForm">
                  <div className="rcaFieldRow">
                    <div className="field">
                      <label className="label">Severity</label>
                      <select
                        className="input"
                        value={rcaDraft.severity}
                        onChange={(e) => updateRca({ severity: e.target.value })}
                      >
                    ...
                    <div className="field">
                      ...
                      <input
                        className="input"
                        value={rcaDraft.detection_source}
                        onChange={(e) => updateRca({ detection_source: e.target.value })}
                        placeholder="Monitoring, user report, automated check"
                      />
                    </div>
                    <div className="field">
                      ...
                      <input
                        ...
                        onChange={(e) => updateRca({ category: e.target.value })}
                        placeholder="Data quality, ingest, pipeline"
                      />
                    </div>
                  </div>
                  <div className="field">
                    ...
                    <textarea
                      className="textarea"
                      value={rcaDraft.impact_summary}
                      onChange={(e) => updateRca({ impact_summary: e.target.value })}
                      placeholder="Who was affected, what was delayed, and scope."
                    />
                  </div>
                  <div className="field">
                    <label className="label">Immediate containment actions</label>
                    <textarea
                      className="textarea"
                      value={rcaDraft.immediate_actions}
                      onChange={(e) => updateRca({ immediate_actions: e.target.value })}
                      placeholder="What was done to reduce impact right away?"
                    />
                  </div>
                  <div className="field">
                    ...
                    <textarea
                      className="textarea"
                      value={rcaDraft.notes}
                      onChange={(e) => updateRca({ notes: e.target.value })}
              ...
              {rcaDraft.stage === "investigation" ? (
                <div className="rcaForm">
                  <div className="field">
                    <label className="label">Symptoms observed</label>
                    <textarea
                      className="textarea"
                      value={rcaDraft.symptoms}
                      onChange={(e) => updateRca({ symptoms: e.target.value })}
                      placeholder="What was seen in logs, reports, or UI?"
                    />
                  </div>
                  <div className="field">
                    <label className="label">Evidence captured</label>
                    <textarea
                      className="textarea"
                      value={rcaDraft.evidence}
                      onChange={(e) => updateRca({ evidence: e.target.value })}
                      placeholder="Links, logs, screenshots, row samples."
                    />
                  </div>
                  <div className="field">
                    <label className="label">Working hypothesis</label>
                    <textarea
                      className="textarea"
                      value={rcaDraft.hypothesis}
                      onChange={(e) => updateRca({ hypothesis: e.target.value })}
                      placeholder="Likely cause(s) before confirmation."
                    />
                  </div>
                  <div className="field">
                    <label className="label">5 Whys</label>
                    <div className="rcaWhys">
                      {rcaDraft.whys.map((why, index) => (
                        <input
                          key={`why-${index}`}
                          className="input"
                          value={why}
              ...
              {rcaDraft.stage === "root_cause" ? (
                <div className="rcaForm">
                  <div className="field">
                    <label className="label">Root cause confirmed</label>
                    <textarea
                      className="textarea"
                      value={rcaDraft.root_cause}
                      onChange={(e) => updateRca({ root_cause: e.target.value })}
                  ...
                  <div className="field">
                    ...
                    <textarea
                      className="textarea"
                      value={rcaDraft.contributing_factors}
                      onChange={(e) => updateRca({ contributing_factors: e.target.value })}
                      placeholder="Secondary contributors that worsened impact."
                    />
                  </div>
                  <div className="field">
                    ...
                    <textarea
                      className="textarea"
                      value={rcaDraft.corrective_action}
                      onChange={(e) => updateRca({ corrective_action: e.target.value })}
                      placeholder="What fixes were applied?"
                    />
                  </div>
                  <div className="field">
                    ...
                    <textarea
                      className="textarea"
                      value={rcaDraft.preventive_action}
                      onChange={(e) => updateRca({ preventive_action: e.target.value })}
                      placeholder="Long-term change to avoid recurrence."
                    />
                  </div>
                </div>
              ) : null}

              {rcaDraft.stage === "verification" ? (
                <div className="rcaForm">
                  <div className="field">
                    <label className="label">Verification steps</label>
                    <textarea
                      className="textarea"
                      value={rcaDraft.verification_steps}
                      onChange={(e) => updateRca({ verification_steps: e.target.value })}
                      placeholder="How we validated the fix."
                    />
                  </div>
                  <div className="field">
                    <label className="label">Monitoring plan</label>
                    <textarea
                      className="textarea"
                      value={rcaDraft.monitoring_plan}
                      onChange={(e) => updateRca({ monitoring_plan: e.target.value })}
                      placeholder="What we will watch after closure."
                    />
                  </div>
                  <div className="field">
                    ...
                    <textarea
                      className="textarea"
                      value={rcaDraft.lessons_learned}
                      onChange={(e) => updateRca({ lessons_learned: e.target.value })}
                      placeholder="Operational learnings for the team."
                    />
                  </div>
                  <div className="rcaFieldRow">
                    <div className="field">
                      <label className="label">Follow-up owner</label>
                      <input
                        className="input"
                        value={rcaDraft.follow_up_owner}
                        onChange={(e) => updateRca({ follow_up_owner: e.target.value })}
                    ...
                    <div className="field">
                      ...
                      <input
                        className="input"
                        type="date"
                        value={rcaDraft.follow_up_date}
                        onChange={(e) => updateRca({ follow_up_date: e.target.value })}
                      />
                    </div>
                  </div>
                  <div className="field">
                    <label className="label">Resolution report</label>
                    <textarea
                      className="textarea"
                      value={rcaDraft.resolution_report}
                      onChange={(e) => updateRca({ resolution_report: e.target.value })}
                      placeholder="Short summary used in the final closure report."
                    />
                  </div>
                </div>
              ) : null}
            ...
            {rcaError ? <div className="errorLine">{rcaError}</div> : null}
            <div className="rcaActions">
              <button className="ghostBtn" type="button" onClick={() => setShowRcaWorkspace(false)}>
                Close
              </button>
              <button className="primaryBtnSmall" type="button" onClick={saveRcaWorkspace} disabled={rcaSaving}>
                {rcaSaving ? "Saving..." : "Save RCA"}
              </button>
            </div>
          </DetailModal>
        ) : null}
  ...
  return (
    <div className="page">
      ...
      <div className="grid2">
        ...
        <Card className={auth.canEditSchedules ? "" : "gridSpanFull"}>
          <div className="formHeader">
            <div>
...
export default function App() {
  ...
  async function handleLogout() {
    ...
    try {
      ...
      try {
        const res = await fetch(`${baseURL()}/api/dashboard-metrics`, { method: "GET" })
        if (!res.ok) throw new Error("metrics failed")
        const data = await res.json().catch(() => ({}))
        const kpis = data?.kpis ?? {}
        const metricToday = Math.max(Number(kpis.todays_runs) || 0, Number(kpis.todays_uploads) || 0)
        if (!mounted) return
        if (metricToday > 0) {
          setTodayUploads(metricToday)
        }
      } catch {
        // ignore
      }
